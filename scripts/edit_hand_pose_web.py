...
<script>
const poseDefs=[
  {label:'X',key:'translation_mm',index:0,min:-500,max:500,unit:'mm'},
  {label:'Y',key:'translation_mm',index:1,min:-500,max:500,unit:'mm'},
  {label:'Z',key:'translation_mm',index:2,min:-500,max:500,unit:'mm'},
  {label:'Roll',key:'euler_deg',index:0,min:-360,max:360,unit:'°'},
  {label:'Pitch',key:'euler_deg',index:1,min:-360,max:360,unit:'°'},
  {label:'Yaw',key:'euler_deg',index:2,min:-360,max:360,unit:'°'},
];
const camDefs=[
  {label:'Azimuth',key:'azimuth',min:-360,max:720,unit:'°',step:1},
  {label:'Elevation',key:'elevation',min:-89,max:89,unit:'°',step:1},
  {label:'Distance',key:'distance',min:.05,max:1.5,unit:'m',step:.01},
  {label:'Look X',key:'lookat',index:0,min:-.5,max:.5,unit:'m',step:.01},
  {label:'Look Y',key:'lookat',index:1,min:-.5,max:.5,unit:'m',step:.01},
  {label:'Look Z',key:'lookat',index:2,min:-.5,max:.5,unit:'m',step:.01},
];
let state,poseDraft,cameraDraft,poseTimer,cameraTimer,mutationSeq=0,renderSeq=0;
function row(def,parent,prefix){
  const suffix=def.index===undefined?def.key:`${def.key}-${def.index}`;
  const id=prefix+suffix.replace('.','-');
  parent.insertAdjacentHTML('beforeend',`<div class="row"><label for="${id}-n">${def.label} <small>${def.unit}</small></label><input id="${id}-r" type="range" min="${def.min}" max="${def.max}"><input id="${id}-n" type="text" inputmode="decimal" autocomplete="off" spellcheck="false"></div>`);
  return {def,r:document.getElementById(id+'-r'),n:document.getElementById(id+'-n')};
}
const poseInputs=poseDefs.map(d=>row(d,document.getElementById('poseRows'),'p-'));
const camInputs=camDefs.map(d=>row(d,document.getElementById('cameraRows'),'c-'));
...
async function api(path,body){
  let r;
  try{r=await fetch(path,{method:body===undefined?'GET':'POST',headers:{'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body)})}
  catch(e){throw new Error(`Request failed: ${e.message}`)}
  const text=await r.text();let j={};
  try{j=text?JSON.parse(text):{}}
  catch(_){throw new Error(`Request failed (${r.status}): invalid server response`)}
  if(!r.ok)throw new Error(j.error||`Request failed (${r.status} ${r.statusText})`);
  return j;
}
function cameraValue(camera,def){return def.index===undefined?camera[def.key]:camera[def.key][def.index]}
function poseValue(pose,def){return pose[def.key][def.index]}
function setPoseValue(pose,def,value){pose[def.key][def.index]=value}
function setCameraValue(camera,def,value){if(def.index===undefined)camera[def.key]=value;else camera[def.key][def.index]=value}
function clonePose(s){return {translation_mm:s.translation_mm.slice(),euler_deg:s.euler_deg.slice()}}
function cloneCamera(camera){return {azimuth:camera.azimuth,elevation:camera.elevation,distance:camera.distance,lookat:camera.lookat.slice()}}
function poseStep(def){return document.getElementById('stepMode').value==='coarse'?10:1}
function syncInputs(s){
  poseDraft=clonePose(s);
  cameraDraft=cloneCamera(s.camera);
  poseInputs.forEach(({def,r,n})=>{const v=poseValue(poseDraft,def);r.step='any';r.dataset.increment=poseStep(def);r.value=v;n.value=v});
  camInputs.forEach(({def,r,n})=>{const v=cameraValue(cameraDraft,def);r.step='any';r.dataset.increment=def.step;r.value=v;n.value=v});
}
function refresh(s,inputs=true){
  state=s;
  document.getElementById('scene').textContent=`${s.physics} · ${s.tip_anchor} anchor`;
  if(inputs)document.getElementById('path').value=s.output_path;
  document.getElementById('anchorLegend').innerHTML=`<i class="dot" style="background:${s.physics==='revolute'?'#ffd90d':'#ff1acc'}"></i>${s.physics==='revolute'?'Revolute anchor':'Point-connect anchor'}`;
  const m=s.metrics;
  document.getElementById('clearance').textContent=m.palm_clearance_mm.toFixed(2)+' mm';
  document.getElementById('thumb').textContent=m.thumb_root_radial_mm.toFixed(2)+' mm';
  document.getElementById('contacts').textContent=m.contact_count+'/3';
  document.getElementById('forces').textContent=m.contact_forces_n.map(x=>x.toFixed(2)).join(' / ')+' N';
  if(inputs)syncInputs(s);
  document.getElementById('render').src='/api/render.png?v='+(++renderSeq);
}
function parseCommitted(raw,def){
  const text=raw.trim();
  if(!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)$/.test(text))throw new Error(`${def.label} must be a complete number`);
  const value=Number(text);
  if(!Number.isFinite(value)||value<def.min||value>def.max)throw new Error(`${def.label} must be between ${def.min} and ${def.max} ${def.unit}`);
  return value;
}
async function sendPose(){
  const body=clonePose(poseDraft),seq=++mutationSeq;
  try{const s=await api('/api/pose',body);if(seq===mutationSeq){refresh(s,false);status('Pose updated.');return true}}
  catch(e){if(seq===mutationSeq)status(e.message,true)}
  return false;
}
async function sendCamera(){
  const body=cloneCamera(cameraDraft),seq=++mutationSeq;
  try{const s=await api('/api/camera',body);if(seq===mutationSeq){refresh(s,false);status('Camera updated.')}}
  catch(e){if(seq===mutationSeq)status(e.message,true)}
}
function queuePose(){clearTimeout(poseTimer);poseTimer=setTimeout(sendPose,90)}
function queueCamera(){clearTimeout(cameraTimer);cameraTimer=setTimeout(sendCamera,90)}
function cancelPending(){
  clearTimeout(poseTimer);clearTimeout(cameraTimer);
  poseTimer=cameraTimer=undefined;
  mutationSeq++;
}
function commitText(control,draft,setValue,send){
  let value;
  try{value=parseCommitted(control.n.value,control.def)}
  catch(e){status(e.message,true);return}
  control.n.value=value;control.r.value=value;
  if(value===poseValueOrCamera(draft,control.def))return;
  setValue(draft,control.def,value);send();
}
function poseValueOrCamera(draft,def){return Array.isArray(draft[def.key])?draft[def.key][def.index]:draft[def.key]}
poseInputs.forEach(control=>{
  const {def,r,n}=control;
  r.addEventListener('input',()=>{const value=Number(r.value);n.value=r.value;setPoseValue(poseDraft,def,value);queuePose()});
  r.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowDown','ArrowRight','ArrowUp'].includes(e.key)){e.preventDefault();const direction=['ArrowLeft','ArrowDown'].includes(e.key)?-1:1,value=Math.min(def.max,Math.max(def.min,Number(r.value)+direction*poseStep(def)));r.value=value;n.value=value;setPoseValue(poseDraft,def,value);queuePose()}});
  n.addEventListener('input',()=>{try{r.value=parseCommitted(n.value,def)}catch(_){}});
  n.addEventListener('change',()=>commitText(control,poseDraft,setPoseValue,sendPose));
  n.addEventListener('blur',()=>commitText(control,poseDraft,setPoseValue,sendPose));
  n.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();commitText(control,poseDraft,setPoseValue,sendPose);n.blur()}});
});
camInputs.forEach(control=>{
  const {def,r,n}=control;
  r.addEventListener('input',()=>{const value=Number(r.value);n.value=r.value;setCameraValue(cameraDraft,def,value);queueCamera()});
  r.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowDown','ArrowRight','ArrowUp'].includes(e.key)){e.preventDefault();const direction=['ArrowLeft','ArrowDown'].includes(e.key)?-1:1,value=Math.min(def.max,Math.max(def.min,Number(r.value)+direction*def.step));r.value=value;n.value=value;setCameraValue(cameraDraft,def,value);queueCamera()}});
  n.addEventListener('input',()=>{try{r.value=parseCommitted(n.value,def)}catch(_){}});
  n.addEventListener('change',()=>commitText(control,cameraDraft,setCameraValue,sendCamera));
  n.addEventListener('blur',()=>commitText(control,cameraDraft,setCameraValue,sendCamera));
  n.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();commitText(control,cameraDraft,setCameraValue,sendCamera);n.blur()}});
});
document.getElementById('stepMode').onchange=()=>poseInputs.forEach(({def,r})=>r.dataset.increment=poseStep(def));
async function stateAction(path,body,message){
  cancelPending();const seq=mutationSeq;
  try{const s=await api(path,body);if(seq===mutationSeq){refresh(s);status(message)}}
  catch(e){if(seq===mutationSeq)status(e.message,true)}
}
document.getElementById('render').onerror=()=>status('Render request failed; check the server log.',true);
document.getElementById('reset').onclick=()=>stateAction('/api/reset',{},'Reset to the editor starting pose.');
document.getElementById('settle').onclick=()=>stateAction('/api/settle',{steps:100},'Settled 100 simulation steps and recomputed contact forces.');
document.getElementById('load').onclick=()=>stateAction('/api/load',{path:document.getElementById('path').value},'Loaded pose; reset now returns to this pose.');
function collectPoseText(){
  try{poseInputs.forEach(control=>{const value=parseCommitted(control.n.value,control.def);control.n.value=value;control.r.value=value;setPoseValue(poseDraft,control.def,value)});return true}
  catch(e){status(e.message,true);return false}
}
async function savePose(overwrite){
  if(!collectPoseText())return;
  clearTimeout(poseTimer);poseTimer=undefined;
  if(!await sendPose())return;
  try{const x=await api('/api/save',{path:document.getElementById('path').value,overwrite});status(`${overwrite?'Overwrote':'Saved'} ${x.saved_path}\nSHA-256 ${x.sha256}`)}
  catch(e){status(e.message,true)}
}
document.getElementById('save').onclick=()=>savePose(false);
document.getElementById('overwrite').onclick=()=>{if(confirm('Explicitly overwrite this existing pose file? This cannot be undone.'))savePose(true)};
